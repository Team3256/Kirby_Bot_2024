...
import frc.robot.subsystems.spindex.ShooterFeederIO;
...
public class ShooterFeederIOTalonFX implements ShooterFeederIO {
    ...
    private final StatusSignal<Double> shooterFeederMotorVoltage = shooterFeederMotor.getMotorVoltage();
    private final StatusSignal<Double> shooterFeederMotorVelocity = shooterFeederMotor.getVelocity();
    private final StatusSignal<Double> shooterFeederMotorStatorCurrent = shooterFeederMotor.getStatorCurrent();
    private final StatusSignal<Double> shooterFeederMotorSupplyCurrent = shooterFeederMotor.getSupplyCurrent();
    private final StatusSignal<Double> shooterFeederMotorTemperature = shooterFeederMotor.getDeviceTemp();

    public ShooterFeederIOTalonFX() {
        PhoenixUtil.applyMotorConfigs(
                shooterFeederMotor, SpindexConstants.feederMotorConfigs, SpindexConstants.flashConfigRetries);
